...
plt.xlabel("Epochs")
plt.ylabel("Accuracy")
plt.title(f"Training Accuracy Over Time ([85*85, 80, 40, 20, 10, 6], ({EPOCHS}, 20, 0.5))")
...
plt.xlabel("Epochs")
plt.ylabel("Cost")
plt.title(f"Inference Cost Over Time ([85*85, 80, 40, 20, 10, 6], ({EPOCHS}, 20, 0.5))")
...
plt.title(f"Inference Learning Rate ŋ Over Time ([85*85, 80, 40, 20, 10, 6], ({EPOCHS}, 20, 0.5))")
# Plot data, setting x values to be epochs (every 1 data points)
plt.plot([*range(0, len(data) * 1, 1)], data)
